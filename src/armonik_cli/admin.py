...
def main():
    ...
    list_session_parser.set_defaults(func=lambda _: session.list_sessions(session_client, args.filter))
    ...
    get_session_parser.set_defaults(func=lambda args: session.check_session(session_client, args.session_ids))
    ...
    list_task_parser.set_defaults(func=lambda args: task.list_tasks(
    task_client,
    task.create_task_filter(args.partition_name, args.session_id, False) if args.partition_name and args.session_id else
    task.create_task_filter(args.partition_name, args.session_id, False) if args.session_id else
    task.create_task_filter(args.partition_name, args.session_id, False) if args.partition_name else
    None
))
    # CHECK TASK
    ...
    get_task_parser.set_defaults(func=lambda args: task.check_task(task_client, args.task_ids))
    ...
    # LIST RESULT
    list_result_parser = result_subparsers.add_parser('list', help='list results with specific filters')
    list_result_parser.add_argument("--completed", dest="filter", default=None, action="store_const", const=ResultFieldFilter.STATUS == RESULT_STATUS_COMPLETED, help="Select running sessions")
    list_result_parser.add_argument("--created",  dest="filter", default=None, action="store_const", const=ResultFieldFilter.STATUS == RESULT_STATUS_CREATED, help="Select cancelled sessions")
    list_result_parser.set_defaults(func=lambda _: result.list_results(result_client, args.filter))
